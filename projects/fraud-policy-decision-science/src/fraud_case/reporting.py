...
The equal-dose factorial interaction averages **${interaction.mean() / 1000:+.1f}K**, with a paired-seed 95% interval of **[${(interaction.mean() - half) / 1000:+.1f}K, ${(interaction.mean() + half) / 1000:+.1f}K]**. The negative interaction indicates overlapping benefits at these doses. A combined intervention can still outperform either component on its own.
...
Python {summary["python"]}; analysis package versions are in `results/summary.json`; `requirements-lock.txt` pins the full dependency environment. All point estimates, scenario comparisons, validation replicates, case inputs and source fingerprints are committed or reproducible from the entry point. Supplied-aggregate experimental uncertainty remains unavailable from the original materials.
"""
