...
The initial question was whether rising fraud losses called for tighter authentication. I began with the loss-rate definition, then followed the evidence through routing, an experiment and recovery operations. Each step changed what the next comparison needed to measure.

Case figures below are supplied scenario evidence. The accompanying simulations have their own populations and numerical results, recorded in [computed results](results.md).

## 1. Establish the population before explaining the increase

The opening dashboard reports 0.32% → 0.47% fraud loss per transaction value over six weeks. My first questions were what the rate measured, when losses entered the numerator, and how card-not-present (CNP) payments differed from other payment types. Transaction occurrence, fraud reporting and loss recognition each answer a different question.
...
CNP transaction count grows 3%, confirmed fraudulent transaction count grows 49%, and mean loss per confirmed fraudulent transaction grows 12%. Within these reported counts, fraudulent incidence grows faster than severity. Exact rate reconciliation still needs the same cohort definitions and value denominators: multiplying 1.49 × 1.12 gives a loss-amount factor of 1.6688, which alone does not reproduce the mature-cohort rate ratio of 0.78/0.44.
...
![Generated recognition and mature-cohort comparisons](../results/figures/10_cohorts.svg)
...
![Generated merchant comparison](../results/figures/02_merchants.svg)
...
A completed-only analysis changes the population. Authentication affects abandonment, completion, fraud mix and severity; each deserves its own denominator. The [experiment note](experiment.md) includes a separate 1,000-per-arm example showing how the completed population can become riskier while the assigned-population loss rate improves.

![Separate teaching example and supplied experiment](../results/figures/04_selection.svg)
...
![Generated information and model comparisons](../results/figures/05_information.svg)
...
![Supplied cross-channel loss sequence](../results/figures/06_displacement.svg)
...
A case-mix decomposition separates composition and liability from recovery-process performance. Exact aggregate fractions yield one accounting bridge; the supplied standardized decomposition of 9.5/6.0/3.5 uses a richer underlying standardization that cannot be recovered from four aggregate totals. The [accounting reconciliation](evidence_provenance.md) records both decompositions. Shapley allocations distribute the interaction among factor contributions.

![Supplied gross-to-net accounting](../results/figures/07_waterfall.svg)
...
The reconstruction uses a detailed case queue for economic comparisons and a smaller randomized state process to check trajectory-weighted estimation. Under the default queue conditions, the adaptive controller stays at high intensity throughout and ties fixed 80%. Under lower arrivals, it releases authentication earlier and loses value against fixed 50%. That makes the threshold and release rule concrete objects to evaluate before adopting an adaptive policy.

![Generated queue and policy trajectories](../results/figures/08_policy_paths.svg)
